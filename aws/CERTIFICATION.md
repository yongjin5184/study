...
* Route 53
    ...
    * Active - Active 장애 조치
        * 모든 리소스를 사용 가능하도록 하려면 해당 구성
        
    * Active - Passive 장애 조치
        * 기본 리소스가 사용 불가능할 경우를 대비해 대기 중에 있도록 구성
...
* EBS
    ...
    * 볼륨 특성
        ...
        * Provisioned IOPS SSD (io1)- 프로비저닝된 IOPS SSD
            * 지연 시간이 짧거나 처리량이 많은 미션 크리티컬 워크로드에 적합한 고성능 SSD 불륨
...
* AWS 인프라 전체의 API 호출을 기록
* API 호출 로그가 S3 버킷에 전달되고 CloudWatch 이벤트에도 제공됨

#### CloudWatch
* AWS 클라우드 리소스 및 AWS 에서 실행하는 응용 프로그램에 대한 모니터링 서비스
